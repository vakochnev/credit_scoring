...
# def generate_explanation_pdf(data: dict, explanation: dict, filename="reports/explanation_report.pdf"):
#     # Убедимся, что путь абсолютный и директория существует
#     filepath = REPORTS_DIR / "explanation_report.pdf"
...
#             <li>{{ line.replace('↑ риск', 'повышает риск').replace('↓ риск', 'понижает риск') }}</li>
#         {% endfor %}
#         </ul>
#
#         <h2>Визуализация вклада признаков</h2>
#         <img src="data:image/png;base64,{{ explanation.shap_image_base64 }}" class="shap-img" alt="SHAP Waterfall Plot">
#     </body>
#     </html>
#     """
#
...
#
#     # Генерируем PDF
#     HTML(string=html_out).write_pdf(target=str(filepath))
#
#     # Возвращаем **абсолютный путь** или относительный, но понятный фронтенду
#     return {"report_path": str(filepath)}


def generate_model_comparison_pdf(results, plot_path, filename="reports/model_comparison_report.pdf"):
    os.makedirs(os.path.dirname(filename), exist_ok=True)
    ...
    <body>
        ...
        <div class="chart">
            ...
            <img src="{{ plot_path }}" alt="Сравнение моделей">
        </div>
