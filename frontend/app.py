...
from shared.config import (
    API_BASE_URL, API_AUTH, REPORT_PATH,
    BACKGROUND_DATA_PATH, ENSEMBLE_MODEL_PATH
)
from shared.models import LoanRequest

# --- Настройка страницы ---
st.set_page_config(page_title="Кредитный скоринг", layout="wide", page_icon="💳")
...
def explain_request(data):
    return requests.post(f"{API_BASE_URL}/explain", json=data, auth=API_AUTH)

def generate_report(data):
    return requests.post(f"{API_BASE_URL}/report", json=data, auth=API_AUTH)

def compare_models():
    return requests.get(f"{API_BASE_URL}/compare", auth=API_AUTH)

def generate_comparison_report():
    return requests.post(f"{API_BASE_URL}/generate-comparison-report", auth=API_AUTH)

# --- 🧠 Загрузка background_data ---
@st.cache_resource
...
# --- Вкладки ---
#tab1, tab2 = st.tabs(["🔍 Прогноз и объяснение", "📊 Сравнение моделей"])
...
with tab1:
    ...
    # Кнопка "Прогнозировать и объяснить"
        ...
        with st.spinner("Выполняется анализ..."):
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Убираем предыдущее состояние PDF
    ...
    if 'prediction_result' in st.session_state:
        ...
        # Показываем кнопку "Скачать", только если отчёт сформирован
    ...
    # --- 📩 Обратная связь (только после прогноза) ---
    st.markdown("---")
        ...
        if st.button("✅ Сохранить обратную связь"):
            ...
            try:
                ...
                if fb_response.status_code == 200:
                    st.success("✅ Обратная связь сохранена для дообучения модели")
                else:
                    st.error(f"❌ Ошибка: {fb_response.json().get('detail', 'Неизвестная ошибка')}")
            except Exception as e:
                st.error("⚠️ Не удалось отправить обратную связь")
                st.exception(e)
    else:
        st.info("Сначала выполните прогноз, чтобы оставить обратную связь.")

# === ВКЛАДКА 2: Сравнение моделей + PDF-отчёт ===
with tab2:
...
#                         )
#                         # Два столбца: Accuracy и AUC
#                         col1, col2 = st.columns(2)
#                         with col1:
#                             st.caption("Точность (Accuracy)")
#                             st.bar_chart(df.set_index("model")["accuracy"])
#                         with col2:
#                             st.caption("AUC-ROC")
#                             st.bar_chart(df.set_index("model")["auc"])
#                     else:
#                         st.warning("AUC не доступен. Обновите `services/model_comparison.py`.")
#                 else:
#                     st.warning("Метрики недоступны. Обучите модели сначала.")
#             except Exception as e:
#                 st.error("⚠️ Не удалось загрузить данные")
